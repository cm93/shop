...
if(judge != 1){
	$(".login-main").empty();
	$(".login-main").append("<h3 align=center style='color:red'>你没有权限访问</h3>")
}
		$("#all").change(function(e){
			fenlei();
		})
	var a = $("option[value=<?php echo $data[2]; ?>]").attr("selected","selected");
	fenlei();
	$("option[value=<?php echo $data[3]; ?>]").attr("selected","selected");
	$("option[value=<?php echo $data[5]; ?>]").attr("selected","selected");
	function fenlei(){
			if ($("#all").val() == "电脑数码"){
						...
						<form action="" method="post" enctype="multipart/form-data" id="imgform" > 
								<div class="register-top-grid">
										...
										<div>
											...
											<input type="text" name="title" id="title" class="required"  required="required" value="<?php echo $data[1]; ?>">
										</div>
										<div id="selecter">
										...
										<div>
											...
											<input type="text" name="name" id="name" class="required"  required="required" value="<?php echo $data[4]; ?>"> 
										</div>
										<div>
										...
										<div>
											...
											<input type="text" name="price" id="price" class="required"  required="required" onkeyup="value=value.replace(/[^\d]/g,'')" style="width:100px;margin-left:75px;text-align:right" value="<?php echo $data[6]; ?>"> 元
										</div>
										<div>
											<span ><label>*</label>二手价格：</span>
											<input type="text" name="newprice" id="newprice" class="required"  required="required" onkeyup="value=value.replace(/[^\d]/g,'')" style="width:100px;margin-left:75px;text-align:right" value="<?php echo $data[8]; ?>"> 元
										</div>
										<div>
											<span ><label>*</label>运费：</span>
											<input type="text" name="sendprice" id="sendprice" class="required"  required="required" onkeyup="value=value.replace(/[^\d]/g,'')" style="width:100px;margin-left:75px;text-align:right" value="<?php echo $data[9]; ?>"> 元
										</div>
										<div >
											...
											<label  for="phonenumber" style="margin-top:4px;left: 150px;position: absolute;color: #666;line-height: 24px;text-align: center;vertical-align: middle;cursor: pointer;white-space: nowrap;font: 18px/1.5 tahoma,arial,'Hiragino Sans GB',\5b8b\4f53,sans-serif;" >手机号：</label>
 											<input type="text" name="phonenumber" id="phonenumber" class="required"  required="required"style="padding-left:72px;width:225px" onkeyup="value=value.replace(/[^\d]/g,'')" value="<?php echo $data[10]; ?>">
 											...
 											<input type="text" name="truename" id="truename" class="required"  required="required"style="padding-left:48px;width:225px" value="<?php echo $data[11]; ?>">
										</div>
										<div >
											<span style="margin:20px 0 20px 0;"><label></label>所在地：<?php echo $data[15]; ?></span>

										</div>
										<div>
											<span ><label></label>物品数量：</span>
											<input type="text" name="sum" id="sum" class="required"  required="required" onkeyup="value=value.replace(/[^\d]/g,'')" style="width:100px;margin-left:75px;text-align:right" value="<?php echo $data[7]; ?>"> 
										</div>
										<div>
										...
										<div id="imgshow">
											<?php foreach (explode(" ",rtrim($data[12])) as $key => $value) { ?>
											<div class="row"><div class="col-sm-6 col-md-4" style="width:500px;"><div class="thumbnail"><img src="<?php echo $value;?> " style="height:150px"><div class="caption"><p><button type="button" class="btn btn-danger" style="margin-left:65px;"  id="del" value="<?php echo $key; ?>">删除</button> </p><input type="hidden" class="img" name="images<?php echo $key; ?>" value="<?php echo $value; ?> "></div></div></div></div>
										...
										<input type="submit" value="修改" id="sub" style="margin-left:75px;"/>
						</form>
